...
//        fillPaint.setStyle(Paint.Style.FILL);
	...
	private void regenerateSpaces(){
		axisYEnd = (int) (getHeight()-(getLabelsSizeInPixels()/2)-50);
		...
		/*
		 * pinta las etiquetas del eje x
		 */
		horizontalSpaceBetweenPoints = ((float) (axisXEnd - axisX))/((float) (valuesMaxAmount-1.0))-minusVal;
	...
	protected void onDraw(Canvas canvas) {
		...
		for(MlkLine line : lines){
	        ...
	        int maxPointY = (int) (axisYEnd-(line.getMaxValue()*verticalSpaceBetweenPoints)-80);
...
//			fillPath.lineTo(axisX, axisYEnd);
		...
		if(getValueLabelsAmount()>0){
			for(int i = 0; i<getValueLabelsAmount()+1; i++){
				...
				if (!getValueLabelFormatter().format(spaceBetweenValues*(i+1)).equals(labelValue) ||  i == getValueLabelsAmount()) {
		...
		canvas.drawLine(axisX, axisY, axisX, axisYEnd+3, axisPaint);
		canvas.drawLine(axisX, axisYEnd+3, axisXEnd, axisYEnd+3, axisPaint);
	}
